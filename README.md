# SliddingMenu
继承`HorizontalScrollView`实现qq的侧滑菜单.。
## 一、qq侧滑菜单原理讲解
<p>我们在使用qq的过程中可以看到用手指左右滑动出现两个界面，其实这两个界面位于同一个布局文件中，只是拉到那一部分就显示那一部分。
在侧滑的时候很容易遇到滑动冲突问题，滑动冲突问题就是当menu菜单中和content菜单中存在listview中，怎样判断滑动的是listview还是
侧滑菜单，在Android中使用HorizontalScrollView可以解决滑动冲突问题。</p>
接下来我们剖析一下详细的代码
## 二、布局文件实现
```
...
```
我们分析一下上面的布局文件，最外面是我们继承`HorizontalScrollView`书写的布局方式，下面会详细讲解。因为在`HorizontalScrollView`中只能包含一个控件，所以我们使用`LinearLayout`。`<include layout="@layout/layout_menu" />`是将`layout_menu`包含进来，`layout_menu`是侧滑菜单左侧菜单区域。
```
<LinearLayout  
    android:layout_width="fill_parent"  
    android:layout_height="fill_parent"  
...
```
这部分比较简单，就是实现每一个条目，请读者自己结合项目学习，请读者先download源码，结合APP分析源码，这个是最快的学习方式。

## 三、 主要代码的实现
`public class SlidingMenu extends HorizontalScrollView`我们先让`SlidingMenu`继承`HorizontalScrollView`
...
```
接下来我们要重写`onMeasure(int widthMeasureSpec, int heightMeasureSpec)`和`onLayout(boolean changed, int l, int t, int r, int b)`, `onMeasure`是计算子控件以及自身的尺寸，`onLayout`是确定空间的额拜访位置。我们看一下详细实现
```
...
protected void onLayout(boolean changed, int l, int t, int r, int b) {
	...
	if(changed){
		this.scrollTo(mMenuWidth, 0); // 设置刚开始的时候隐藏菜单scrollTo(mMenuWidth, 0);
		once = true;
	}
